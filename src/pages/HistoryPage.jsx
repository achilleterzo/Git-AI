...
export default function HistoryPage({ directory, projects, choose, selectProject, removeProjectOption, active, stop, resume, defaultPathIcon, currentBranch, gitLfs, onBranchSwitch, incomingCommits, outgoingCommits, runGitRemote, requestPush, runOperation, setErrorModal, aiEnabled, refreshPendingCommitCount, showDiff }) {
  ...
  async function refresh() { try { const [history, pending] = await Promise.all([window.directoryAPI.getHistory({ offset: 0, limit: 15 }), window.directoryAPI.getPendingCommits()]); const pendingHashes = new Set(pending.map(commit => commit.hash)); setPendingCommits(pending); setCommits(history.filter(commit => !pendingHashes.has(commit.hash))); setHasMore(history.length === 15); setExpandedCommit(''); setCommitFiles({}) } catch (error) { setErrorModal(error.message) } }
  async function loadMore() { if (loadingMore || !hasMore) return; setLoadingMore(true); try { const next = await window.directoryAPI.getHistory({ offset: commits.length + pendingCommits.length, limit: 15 }); setCommits(value => [...value, ...next]); setHasMore(next.length === 15) } catch (error) { setErrorModal(error.message) } finally { setLoadingMore(false) } }
  ...
  function renderCommit(commit) { const files = commitFiles[commit.hash] || []; return <article className={`history-card ${expandedCommit === commit.hash ? 'expanded' : ''}`} key={commit.hash} onClick={() => toggleFiles(commit)}><div className="history-card-main"><div className="history-commit-meta"><code>{commit.shortHash}</code><span>{formatDate(commit.date)}</span><span>{commit.author}</span></div><h3>{commit.message}</h3><div className="history-tags">{(commit.tags || []).map(tag => <span className="history-tag" key={tag} onClick={event => event.stopPropagation()}><span>{tag}</span><button title={`Delete ${tag}`} onClick={() => deleteTag(tag)}>×</button></span>)}<button className="history-add-tag" onClick={event => { event.stopPropagation(); openCommitTag(commit) }}>＋ Tag commit</button></div></div>{expandedCommit === commit.hash && <div className="history-files" onClick={event => event.stopPropagation()}>{filesLoading === commit.hash ? <span className="muted">Loading files…</span> : files.length ? files.map(item => <button className="history-file" key={item.file} onClick={() => openCommitDiff(commit, item.file)}><code>{item.status}</code><span>{item.file}</span><small>View diff</small></button>) : <span className="muted">No files found.</span>}</div>}</article> }
