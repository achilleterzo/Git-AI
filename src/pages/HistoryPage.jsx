...
export default function HistoryPage({ directory, projects, choose, selectProject, removeProjectOption, active, stop, resume, defaultPathIcon, currentBranch, gitLfs, onBranchSwitch, incomingCommits, outgoingCommits, hasUpstream, runGitRemote, requestPush, runOperation, setErrorModal, aiEnabled, refreshPendingCommitCount, showDiff }) {
  ...
  const [historyOffset, setHistoryOffset] = useState(15)
  const [fileExpanded, setFileExpanded] = useState({})

  async function refresh() { try { const [history, pending] = await Promise.all([window.directoryAPI.getHistory({ offset: 0, limit: 15 }), window.directoryAPI.getPendingCommits()]); const pendingHashes = new Set(pending.map(commit => commit.hash)); setPendingCommits(pending); setCommits(history.filter(commit => !pendingHashes.has(commit.hash))); setHistoryOffset(15); setHasMore(history.length === 15); setExpandedCommit(''); setCommitFiles({}); setFileExpanded({}) } catch (error) { setErrorModal(error.message) } }
  async function loadMore() { if (loadingMore || !hasMore) return; setLoadingMore(true); const offset = historyOffset; try { const next = await window.directoryAPI.getHistory({ offset, limit: 15 }); setCommits(value => [...value, ...next]); setHistoryOffset(offset + next.length); setHasMore(next.length === 15) } catch (error) { setErrorModal(error.message) } finally { setLoadingMore(false) } }
  async function toggleFiles(commit) { if (expandedCommit === commit.hash) return setExpandedCommit(''); setExpandedCommit(commit.hash); if (commitFiles[commit.hash]) return; setFilesLoading(commit.hash); try { const files = await window.directoryAPI.getCommitFiles(commit.hash); setCommitFiles(value => ({ ...value, [commit.hash]: files })) } catch (error) { setErrorModal(error.message) } finally { setFilesLoading('') } }
  async function openCommitDiff(commit, file) { try { const diff = await window.directoryAPI.getCommitDiff(commit.hash, file); showDiff(`${commit.shortHash} · ${file}`, diff) } catch (error) { setErrorModal(error.message) } }
  useEffect(() => { const onScroll = () => { if (window.innerHeight + window.scrollY >= document.documentElement.scrollHeight - 500) loadMore() }; window.addEventListener('scroll', onScroll); return () => window.removeEventListener('scroll', onScroll) }, [commits.length, hasMore, loadingMore])
  useEffect(() => { if (!loadingMore && hasMore && document.documentElement.scrollHeight <= window.innerHeight + 500) loadMore() }, [commits.length, hasMore, loadingMore])
  ...
  function renderCommit(commit) { const files = commitFiles[commit.hash] || []; const folderPaths = [...new Set(files.flatMap(item => { const parts = item.file.replaceAll('\\', '/').split('/').filter(Boolean); return parts.slice(0, -1).map((_, index) => parts.slice(0, index + 1).join('/')) }))]; const expandedFiles = new Set(fileExpanded[commit.hash] || []); const toggleFileFolder = path => setFileExpanded(value => ({ ...value, [commit.hash]: expandedFiles.has(path) ? [...expandedFiles].filter(item => item !== path) : [...expandedFiles, path] })); return <article className={`history-card ${expandedCommit === commit.hash ? 'expanded' : ''}`} key={commit.hash} onClick={() => toggleFiles(commit)}><div className="history-card-main"><div className="history-commit-meta"><code>{commit.shortHash}</code><span>{formatDate(commit.date)}</span><span>{commit.author}</span></div><h3>{commit.message}</h3><div className="history-tags">{(commit.tags || []).map(tag => <span className="history-tag" key={tag} onClick={event => event.stopPropagation()}><span>{tag}</span><button title={`Delete ${tag}`} onClick={() => deleteTag(tag)}>×</button></span>)}<button className="history-add-tag" onClick={event => { event.stopPropagation(); openCommitTag(commit) }}>＋ Tag commit</button></div></div>{expandedCommit === commit.hash && <div className="history-files" onClick={event => event.stopPropagation()}>{filesLoading === commit.hash ? <span className="muted">Loading files…</span> : files.length ? <FilesTable changes={files.map(item => ({ file: item.file, status: item.status === 'A' ? 'Added' : item.status === 'D' ? 'Deleted' : 'Modified' }))} fileIndexing={{ status: 'idle' }} query="" onQueryChange={() => {}} selected={new Set()} expanded={expandedFiles} toggleFolder={toggleFileFolder} toggleSelection={() => {}} expandAllFolders={() => setFileExpanded(value => ({ ...value, [commit.hash]: folderPaths }))} collapseAllFolders={() => setFileExpanded(value => ({ ...value, [commit.hash]: [] }))} openDiff={file => openCommitDiff(commit, file)} compact readOnly title="Files in commit" emptyMessage="No files found." /> : <span className="muted">No files found.</span>}</div>}</article> }
  return <div className="history-page">
    <ProjectToolbar {...{ directory, projects, choose, selectProject, removeProjectOption, active, stop, resume, defaultPathIcon }} />
    <RepositoryActionsBar {...{ directory, currentBranch, gitLfs, onBranchSwitch, incomingCommits, outgoingCommits, hasUpstream, runGitRemote: refreshAfterRemote, requestPush, selected: new Set(), gitBusy: releaseAiBusy, onTagCommit: openReleaseTag, showPull: true, showPush: true, showTag: true }} />
    ...
    <section className="history-section"><div className="history-section-head"><div><h2>Commit history</h2><p className="muted">Browse commits and manage their Git tags.</p></div><button className="ghost" onClick={refresh}>Refresh</button></div><div className="history-list">{commits.length ? commits.map(renderCommit) : <div className="empty"><div>⌁</div><h3>No commits found</h3><p>This repository does not have commit history yet.</p></div>}{loadingMore && <div className="history-loading">Loading more commits…</div>}</div></section>
