    ...
    <div className="settings-modal" onClick={event => event.stopPropagation()}>
      ...
      <section className="settings-body">
        ...
        {settings.aiEnabled && <div className="settings-ai-fields">
          ...
          {nativeProvider ? <>
            <div className="settings-provider-status"><div><strong>{providerDefinition.label} client</strong><small>{statusText}. Pulse does not store or handle OAuth tokens.</small></div><div className="settings-provider-actions"><button className={`ghost ${aiStatus?.authenticated ? 'settings-connected' : ''}`} onClick={() => refreshAiStatus(provider)} disabled={aiStatusLoading}>{aiStatusLoading ? 'Checking…' : 'Refresh status'}</button>{!aiStatus?.authenticated && <button className="primary" onClick={loginAiProvider} disabled={!aiStatus?.installed || aiStatusLoading}>Login</button>}</div></div>
