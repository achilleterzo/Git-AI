...
function App() {
  ...
  const [plannedCommits, setPlannedCommits] = useState(null)
  const [expandedPlannedCommit, setExpandedPlannedCommit] = useState(null)
  ...
  useEffect(() => { window.directoryAPI.onUpdate(data => {
    ...
    setHasUpstream(Boolean(data.hasUpstream))
  ...
  async function openSelectedDirectory(path, project) { const metadata = project || { path }; const targetView = view === 'home' ? 'changes' : view; const previousView = view; setDirectory(path); setProjectIcon(metadata.icon || null); setFileIndexing({ status: 'idle', error: null }); setCurrentBranch(''); setIncomingCommits(0); setOutgoingCommits(0); setHasUpstream(false); setGitLfs(Boolean(metadata.gitLfs)); setActive(false); setStarting(true); setChanges([]); setSelected(new Set()); setStashes([]); setEmptyDirectory(''); setCheckoutRemote(''); setView(targetView); try { const watchResult = await window.directoryAPI.startWatching(path); if (watchResult?.code === 'WATCH_SUPERSEDED') return false; if (watchResult?.ok === false && watchResult.code === 'NOT_A_GIT_REPOSITORY') { setProjects(normalizeProjects(await window.directoryAPI.addProject(metadata))); setActive(false); setStarting(false); setEmptyDirectory(path); setProjectModal(false); return false } setProjects(normalizeProjects(await window.directoryAPI.addProject(metadata))); return true } catch (error) { const message = String(error?.message || error || ''); setStarting(false); setView(previousView); setErrorModal(message); return false } }
  ...
  function expandAllFolders(paths = null) { setExpanded(value => new Set([...value, ...(paths || changes.flatMap(change => { const parts = change.file.replaceAll('\\', '/').split('/').filter(Boolean); return parts.slice(0, -1).map((_, index) => parts.slice(0, index + 1).join('/')) }))])) }
  ...
  async function commitSelected(pushAfter = false) { const amend = pendingOperation === 'amend'; setAiBusy(true); setAiError(''); try { await runOperation(amend ? 'Amending commit…' : 'Creating commit…', () => window.directoryAPI.commitSelected([...selected], aiMessage, amend)); setAiMessage(''); setSelected(new Set()); setLastEvent(amend ? 'Commit amended' : 'Commit completed'); if (pushAfter) await runGitRemote('push', true) } catch (error) { setAiError(error.message); setErrorModal(error.message) } finally { setAiBusy(false) } }
  async function commitPlanned(pushAfter = false) { setAiBusy(true); setAiError(''); try { const result = await runOperation('Executing planned commits…', () => window.directoryAPI.commitPlanned({ commits: plannedCommits })); if (result?.ok === false) { const completed = result.commits?.length || 0; setPlannedCommits(items => (items || []).slice(completed)); setExpandedPlannedCommit(null); const detail = `${completed} planned commit${completed === 1 ? '' : 's'} completed before the error. ${result.error || 'The remaining commits were not created.'}`; setAiError(detail); setErrorModal(detail); setLastEvent('Planned commits partially completed'); return } setAiMessage(''); setPlannedCommits(null); setSelected(new Set()); setLastEvent('Planned commits completed'); if (pushAfter) await runGitRemote('push', true) } catch (error) { setAiError(error.message); setErrorModal(error.message) } finally { setAiBusy(false) } }
  ...
  async function requestPush() { if (!directory || !currentBranch || gitBusy || (hasUpstream && outgoingCommits < 1)) return; setPushConfirmation(true) }
  async function runGitRemote(action, confirmed = false) { if (action === 'push' && !confirmed) { if (!directory || !currentBranch || gitBusy || (hasUpstream && outgoingCommits < 1)) return; setPushConfirmation(true); return } setGitBusy(true); try { const result = await runOperation(action === 'push' ? (hasUpstream ? 'Pushing changes…' : 'Publishing branch…') : 'Pulling changes…', () => action === 'pull' ? window.directoryAPI.gitPull() : window.directoryAPI.gitPush(), { trackProgress: true }); setLastEvent(result || `${action === 'push' ? 'Push' : 'Pull'} completed`) } catch (error) { setErrorModal(error.message) } finally { setGitBusy(false) } }
  ...
  return (
    <main>
      ...
      <section className="content">
        ...
        {settingsOpen && <SettingsModal settings={settings} setSettings={setSettings} models={models} modelsLoading={modelsLoading} loadModels={loadModels} aiStatus={aiStatus} aiStatusLoading={aiStatusLoading} refreshAiStatus={refreshAiStatus} loginAiProvider={loginAiProvider} aiError={aiError} save={saveAiSettings} onClose={() => setSettingsOpen(false)} />}
        ...
        {pushConfirmation && <ConfirmationModal title={hasUpstream ? 'Confirm push' : 'Publish branch?'} message={hasUpstream ? 'Review the push before sending your local commits to the remote repository.' : 'This branch is local. Publishing it will create the matching branch on origin and configure tracking.'} details={<><div><span>BRANCH</span><strong>{currentBranch || 'Unknown branch'}</strong></div><div><span>COMMITS TO PUSH</span><strong>{outgoingCommits}</strong></div><div><span>DIRECTORY</span><strong>{directory}</strong></div></>} confirmLabel={hasUpstream ? 'Push commits' : 'Publish branch'} onCancel={() => setPushConfirmation(false)} onConfirm={confirmPush} />}
        ...
        {(aiMessage || aiError) && (
          <div className="modal-backdrop" onClick={() => {
            if (!aiBusy) {
              setAiMessage('')
              setPlannedCommits(null)
              setExpandedPlannedCommit(null)
              setAiError('')
            }
          }}>
            <div className={`modal ${pendingOperation === 'planned-commits' ? 'planned-commits-modal' : ''}`} onClick={event => event.stopPropagation()}>
              <h2>{pendingOperation === 'planned-commits' ? 'Planned Commits' : pendingOperation === 'stash-merge' ? 'Merge stash message' : pendingOperation === 'stash' ? 'Stash message' : pendingOperation === 'amend' ? 'Amend commit message' : 'Commit message'}</h2>
              {pendingOperation === 'planned-commits' ? (
                <div className="history-list planned-commits-list">
                  {(plannedCommits || []).map((commit, index) => {
                    const isExpanded = expandedPlannedCommit === index
                    return (
                      <article className={`history-card planned-commit-card ${isExpanded ? 'expanded' : ''}`} key={index}>
                        <button type="button" className="planned-commit-summary" onClick={() => setExpandedPlannedCommit(isExpanded ? null : index)}>
                          <span className="planned-commit-title">
                        ...
                        {isExpanded && (
                          <div className="planned-commit-details">
                            <label>Commit message
                              <input value={commit.message} onChange={event => setPlannedCommits(items => items.map((item, itemIndex) => itemIndex === index ? { ...item, message: event.target.value } : item))} />
                            </label>
                            <FilesTable
                              changes={commit.files.map(file => changes.find(change => change.file === file) || { file, status: 'Modified', code: ' M' })}
                              fileIndexing={{ status: 'idle' }}
                              query=""
                              onQueryChange={() => {}}
                              selected={new Set()}
                              expanded={expanded}
                              ...
                              openDiff={openDiff}
                              title="Files included in this commit"
                              compact
                              readOnly
                            />
                          </div>
                ...
                <textarea value={aiMessage} onChange={event => setAiMessage(event.target.value)} />
              )}
              <div className="modal-actions">
                <button className="ghost" onClick={() => { setAiMessage(''); setPlannedCommits(null); setExpandedPlannedCommit(null); setAiError('') }}>Close</button>
                {aiError && <button className="ghost" onClick={() => pendingOperation === 'stash-merge' ? generateStashMergeMessage() : generateCommitMessage(pendingOperation)} disabled={aiBusy}>{aiBusy ? 'Retrying…' : 'Retry'}</button>}
                {pendingOperation === 'planned-commits' ? <><button className="primary" onClick={() => commitPlanned(false)} disabled={aiBusy || !(plannedCommits || []).length || plannedCommits.some(commit => !commit.message.trim() || !commit.files.length)}>Commit all</button><button className="primary" onClick={() => commitPlanned(true)} disabled={aiBusy || !(plannedCommits || []).length || plannedCommits.some(commit => !commit.message.trim() || !commit.files.length)}>Commit all &amp; Push</button></> : pendingOperation === 'stash-merge' ? <button className="primary" onClick={mergeStashes} disabled={aiBusy || !aiMessage.trim()}>Merge stashes</button> : pendingOperation === 'stash' ? <button className="primary" onClick={stashSelected} disabled={aiBusy || !aiMessage.trim()}>Stash</button> : <>{<button className="primary" onClick={() => commitSelected(false)} disabled={aiBusy || !aiMessage.trim()}>{pendingOperation === 'amend' ? 'Amend commit' : 'Commit'}</button>}{pendingOperation !== 'amend' && <button className="primary" onClick={() => commitSelected(true)} disabled={aiBusy || !aiMessage.trim()}>Commit &amp; Push</button>}</>}
              </div>
