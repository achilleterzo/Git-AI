  ...
  return (
    <main>
      ...
      <section className="content">
        ...
        {(aiMessage || aiError) && <div className="modal-backdrop" onClick={() => { if (!aiBusy) { setAiMessage(''); setAiError('') } }}><div className="modal" onClick={event => event.stopPropagation()}><h2>{pendingOperation === 'stash-merge' ? 'Merge stash message' : pendingOperation === 'stash' ? 'Stash message' : pendingOperation === 'amend' ? 'Amend commit message' : 'Commit message'}</h2><textarea value={aiMessage} onChange={event => setAiMessage(event.target.value)} /><div className="modal-actions"><button className="ghost" onClick={() => { setAiMessage(''); setAiError('') }}>Close</button>{aiError && <button className="ghost" onClick={() => pendingOperation === 'stash-merge' ? generateStashMergeMessage() : generateCommitMessage(pendingOperation)} disabled={aiBusy}>{aiBusy ? 'Retrying…' : 'Retry'}</button>}{pendingOperation === 'stash-merge' ? <button className="primary" onClick={mergeStashes} disabled={aiBusy || !aiMessage.trim()}>Merge stashes</button> : pendingOperation === 'stash' ? <button className="primary" onClick={stashSelected} disabled={aiBusy || !aiMessage.trim()}>Stash</button> : <button className="primary" onClick={commitSelected} disabled={aiBusy || !aiMessage.trim()}>{pendingOperation === 'amend' ? 'Amend commit' : 'Commit'}</button>}</div>{aiError && <p className="modal-error">{aiError}</p>}</div></div>}
      </section>
    </main>
  )
}
