...
function App() {
  ...
  async function choose() { setProjectEditingPath(''); setProjectDraft({ path: '', name: '', icon: null }); setProjectModal(true) }
  ...
  async function browseProjectIcon() { try { const icon = await window.directoryAPI.chooseProjectIcon(); if (icon) setProjectDraft(value => ({ ...value, icon })) } catch (error) { setErrorModal(error.message) } }
  async function editProject(project) { setProjectEditingPath(project.path); setProjectDraft({ path: project.path, name: project.name || projectName(project), icon: project.icon || null }); setProjectModal(true) }
  async function saveProject() { if (!projectDraft.path) return; const editing = Boolean(projectEditingPath); const project = { ...projectDraft, name: projectDraft.name.trim() || projectName({ path: projectDraft.path }), ...(editing ? {} : { lastOpened: Date.now() }) }; setLoading(editing ? 'Saving project…' : 'Loading project…'); try { if (editing) { if (projectEditingPath !== projectDraft.path) { await window.directoryAPI.removeProject(projectEditingPath); setProjects(normalizeProjects(await window.directoryAPI.addProject(project))) } else setProjects(normalizeProjects(await window.directoryAPI.updateProject(projectEditingPath, project))) } else { await window.directoryAPI.startWatching(projectDraft.path); setProjects(normalizeProjects(await window.directoryAPI.addProject(project))) } setProjectModal(false); setProjectEditingPath(''); if (!editing) setView('changes') } catch (error) { setErrorModal(error.message) } finally { setLoading('') } }
  async function selectProject(option) { if (option?.value) { setLoading('Loading project…'); try { await window.directoryAPI.startWatching(option.value); setProjects(normalizeProjects(await window.directoryAPI.addProject({ path: option.value, lastOpened: Date.now() }))) } catch (error) { setErrorModal(error.message) } finally { setLoading('') } } }
  ...
  async function commitSelected() { setAiBusy(true); setLoading('Creating commit…'); setAiError(''); try { await window.directoryAPI.commitSelected([...selected], aiMessage); setAiMessage(''); setSelected(new Set()); setLastEvent('Commit completed'); const update = await window.directoryAPI.refresh(); if (update?.changes) setChanges(update.changes); if (update?.files) setFiles(update.files) } catch (error) { setAiError(error.message); setErrorModal(error.message) } finally { setAiBusy(false); setLoading('') } }
  async function stashSelected() { setAiBusy(true); setLoading('Creating stash…'); setAiError(''); try { await window.directoryAPI.stashSelected([...selected], aiMessage); setAiMessage(''); setSelected(new Set()); setLastEvent('Stash created'); const update = await window.directoryAPI.refresh(); if (update?.changes) setChanges(update.changes); if (update?.files) setFiles(update.files); setStashes(await window.directoryAPI.getStashes()) } catch (error) { setAiError(error.message); setErrorModal(error.message) } finally { setAiBusy(false); setLoading('') } }
  ...
  return <main>
    ...
    <section className="content">{view === 'home' ? <div className="home-page"><header><div><p className="eyebrow">WORKSPACE / HOME</p><h1>Projects</h1><p className="muted">Open one of your saved Git repositories.</p></div></header><div className="home-project-grid"><button className="home-project-card new-project-card" onClick={choose}><span className="new-project-icon">＋</span><span>New project</span><small>Choose a directory</small></button>{projects.slice().sort((a, b) => (b.lastOpened || 0) - (a.lastOpened || 0)).map(project => <div className={`home-project-card project-card ${project.path === directory ? 'active' : ''}`} key={project.path} title={project.path} onClick={() => openHomeProject(project)}><img src={project.icon || defaultPathIcon} alt="" /><span>{projectName(project)}</span><small>Open project</small><button className="project-edit-button" title="Edit project" onClick={event => { event.stopPropagation(); editProject(project) }}>Edit</button></div>)}</div></div> : <><header><div><p className="eyebrow">WORKSPACE / MONITOR</p><h1>Directory watcher</h1><p className="muted">Monitor file changes in real time from the shell.</p></div><div className={`status ${active ? 'online' : ''}`}><i />{active ? 'WATCHING' : 'IDLE'}</div></header>
      ...
      </>} {updateRelease && updateNoticeVisible && <div className="update-banner"><div><strong>Update available: v{updateRelease.version}</strong><span>You are using v{appVersion}.</span></div><button className="primary" onClick={() => window.directoryAPI.openRelease(updateRelease.url)}>Download latest build</button><button className="update-close" aria-label="Dismiss update" onClick={() => setUpdateNoticeVisible(false)}>×</button></div>} {loading && <div className="loading-backdrop"><div className="loader-card"><span className="spinner" /><strong>{loading}</strong></div></div>}
