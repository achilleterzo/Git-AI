...
function App() {
  ...
  async function executeBranchSwitch(request, stash) { setBranchSwitchRequest(null); setLoading(stash ? 'Stashing changes and switching branch…' : 'Switching branch…'); pendingRefreshRef.current = 'branch-switch'; try { await window.directoryAPI.switchBranch({ ...request, stash }); pendingRefreshRef.current = ''; setLoading(''); setSelected(new Set()) } catch (error) { pendingRefreshRef.current = ''; setErrorModal(error.message) } finally { if (!pendingRefreshRef.current) setLoading('') } }
