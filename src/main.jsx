...
function App() {
  ...
  function clearOperation(id) { const current = operationRef.current; if (current.id !== id) return; if (current.timer) clearTimeout(current.timer); operationRef.current = { id, timer: null }; setLoading('') }
  async function runOperation(label, task) { if (operationRef.current.timer) clearTimeout(operationRef.current.timer); const id = operationRef.current.id + 1; operationRef.current = { id, timer: null }; setLoading(label); operationRef.current.timer = setTimeout(() => { if (operationRef.current.id !== id) return; operationRef.current = { id, timer: null }; setLoading(''); setErrorModal(`${label.replace(/…$/, '')} is taking longer than ${Math.round(OPERATION_TIMEOUT_MS / 1000)}s and is still running in the background. Check the operation console for details.`) }, OPERATION_TIMEOUT_MS); try { return await task() } finally { clearOperation(id) } }
  async function openSelectedDirectory(path, project) { const metadata = project || { path }; setDirectory(path); setProjectIcon(metadata.icon || null); setCurrentBranch(''); setIncomingCommits(0); setOutgoingCommits(0); setGitLfs(Boolean(metadata.gitLfs)); setActive(false); setFiles([]); setChanges([]); setSelected(new Set()); setStashes([]); setEmptyDirectory(''); setCheckoutRemote(''); try { await window.directoryAPI.startWatching(path); setProjects(normalizeProjects(await window.directoryAPI.addProject(metadata))); setView('changes'); return true } catch (error) { const message = String(error?.message || error || ''); if (error?.code === 'EMPTY_DIRECTORY_NOT_REPOSITORY' || message.includes('empty and is not a Git repository')) { setProjects(normalizeProjects(await window.directoryAPI.addProject(metadata))); setActive(false); setEmptyDirectory(path); setProjectModal(false); setView('changes'); return false } setErrorModal(message); return false } }
  ...
  async function selectProject(option) { if (!option?.value) return; try { await runOperation('Loading project…', () => openSelectedDirectory(option.value, { path: option.value, lastOpened: Date.now() })) } catch (error) { setErrorModal(String(error?.message || error)) } }
  ...
  async function resume() { if (!directory) return; try { await runOperation('Starting monitor…', () => window.directoryAPI.startWatching(directory)) } catch (error) { setErrorModal(String(error?.message || error)) } }
  ...
  async function runGitRemote(action, confirmed = false) { if (action === 'push' && !confirmed) { if (!directory || gitBusy || outgoingCommits < 1) return; setPushConfirmation(true); return } setGitBusy(true); try { const result = await runOperation(action === 'push' ? 'Pushing changes…' : 'Pulling changes…', () => action === 'pull' ? window.directoryAPI.gitPull() : window.directoryAPI.gitPush()); setLastEvent(result || `${action === 'push' ? 'Push' : 'Pull'} completed`) } catch (error) { setErrorModal(error.message) } finally { setGitBusy(false) } }
      ...
      <section className="content">
        {view === 'home' ? (
          ...
          <>
            ...
            {view === 'changes' ? <ChangesPage {...pageProps} /> : view === 'stash' ? <StashPage {...pageProps} stashes={stashes} /> : <LfsPage directory={directory} gitLfs={gitLfs} loading={loading} runOperation={runOperation} setErrorModal={setErrorModal} />}
          </>
        )}
