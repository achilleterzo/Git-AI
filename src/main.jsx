...
function App() {
  ...
  async function selectProject(event) { const selected = event.target.value; if (selected) await window.directoryAPI.startWatching(selected) }
  ...
  return <main>
    <aside><div className="brand"><span>◈</span><div><strong>Pulse</strong><small>DIRECTORY WATCHER</small></div></div><div className="side-label">MONITOR</div><div className="nav active">▦ <span>File changes</span></div><div className="nav">⌁ <span>Shell activity</span></div><div className="side-footer">Electron desktop<br /><span>v1.0.0 · ready</span></div></aside>
    <section className="content"><header><div><p className="eyebrow">WORKSPACE / MONITOR</p><h1>Directory watcher</h1><p className="muted">Monitor file changes in real time from the shell.</p></div><div className={`status ${active ? 'online' : ''}`}><i />{active ? 'WATCHING' : 'IDLE'}</div></header>
      <div className="toolbar"><select className="project-select" value={directory} onChange={selectProject}><option value="">Recent projects…</option>{projects.map(project => <option key={project} value={project}>{project}</option>)}</select><div className="path">⌂ <span>{directory || 'No directory selected'}</span></div><button className="primary" onClick={choose}>＋ Choose directory</button>{directory && <button className="ghost" onClick={removeProject}>Remove</button>}{active && <button className="ghost" onClick={stop}>Stop</button>}</div>
      ...
      <div className="panel"><div className="panel-head"><div><h2>Changes to commit</h2><p>{visibleChanges.length} Git files · {selected.size} selected</p></div><div className="panel-actions"><input placeholder="Search files..." value={query} onChange={e => setQuery(e.target.value)} /><button className="ai-button" disabled={!selected.size || aiBusy} onClick={generateCommitMessage}>{aiBusy ? 'Generating…' : '✦ Generate commit'}</button></div></div><div className="tree-head"><span className="head-selection"><SelectionBox checked={allSelected} indeterminate={allPartial} onChange={() => toggleSelection(allPaths)} /> FILE / DIRECTORY</span><span>STATUS</span><span>CODE</span></div>{changes.length ? renderTree(tree) : <div className="empty"><div>✓</div><h3>Working tree clean</h3><p>No added, deleted, or modified files to commit.</p>{pushPending && <button className="push-cta" disabled={gitBusy} onClick={() => runGitRemote('push')}>↑ Push local commits</button>}</div>}</div>
