...
const projectName = directory => directory.split(/[\\/]/).filter(Boolean).at(-1) || directory
...
function App() {
  ...
  async function choose() { const selected = await window.directoryAPI.chooseDirectory(); if (selected) { setLoading('Loading project…'); try { await window.directoryAPI.startWatching(selected); setProjects(normalizeProjects(await window.directoryAPI.addProject(selected))) } catch (error) { setErrorModal(error.message) } finally { setLoading('') } } }
  ...
  return <main>
    <aside><div className="brand"><span>◈</span><div><strong>Pulse</strong><small>GIT AI</small></div></div><div className="side-label">HOME</div><div className={`nav ${view === 'home' ? 'active' : ''}`} onClick={() => setView('home')}>⌂ <span>Home</span></div><div className="side-label monitor-label">MONITOR</div><div className={`nav ${view === 'changes' ? 'active' : ''}`} onClick={() => setView('changes')}>▦ <span>File changes</span></div><div className={`nav ${view === 'stash' ? 'active' : ''}`} onClick={() => setView('stash')}>▣ <span>Stash</span></div><div className="side-footer">Electron desktop<br /><span>v1.0.0 · ready</span></div></aside>
    <section className="content">{view === 'home' ? <div className="home-page"><header><div><p className="eyebrow">WORKSPACE / HOME</p><h1>Projects</h1><p className="muted">Open one of your saved Git repositories.</p></div></header><div className="home-project-grid">{projects.length ? projects.map(project => <button className={`home-project-card ${project.path === directory ? 'active' : ''}`} key={project.path} title={project.path} onClick={() => openHomeProject(project)}><img src={project.icon || defaultPathIcon} alt="" /><span>{projectName(project.path)}</span><small>Open project</small></button>) : <div className="empty"><div>—</div><h3>No saved projects</h3><p>Choose a directory from the monitor to add it here.</p></div>}</div></div> : <><header><div><p className="eyebrow">WORKSPACE / MONITOR</p><h1>Directory watcher</h1><p className="muted">Monitor file changes in real time from the shell.</p></div><div className={`status ${active ? 'online' : ''}`}><i />{active ? 'WATCHING' : 'IDLE'}</div></header>
      {view === 'changes' ? <>
