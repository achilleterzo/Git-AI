...
function App() {
  ...
  async function choose() { const selected = await window.directoryAPI.chooseDirectory(); if (selected) { setLoading('Loading project…'); try { setProjects(normalizeProjects(await window.directoryAPI.addProject(selected))); await window.directoryAPI.startWatching(selected) } finally { setLoading('') } } }
  async function selectProject(option) { if (option?.value) { setLoading('Loading project…'); try { await window.directoryAPI.startWatching(option.value) } finally { setLoading('') } } }
  ...
  return <main>
    <aside><div className="brand"><span>◈</span><div><strong>Pulse</strong><small>GIT AI</small></div></div><div className="side-label">MONITOR</div><div className={`nav ${view === 'changes' ? 'active' : ''}`} onClick={() => setView('changes')}>▦ <span>File changes</span></div><div className={`nav ${view === 'stash' ? 'active' : ''}`} onClick={() => setView('stash')}>▣ <span>Stash</span></div><div className="side-footer">Electron desktop<br /><span>v1.0.0 · ready</span></div></aside>
